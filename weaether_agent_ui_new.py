...
from weather import  Deps, weather_agent
...
async def stream_from_agent(prompt: str, chatbot: list[dict], past_messages: list):
    ...
    async with weather_agent.run_stream(
        ...
        async for message in stream_text:
            chatbot[-1]['content'] = message[0].parts[0].content
...
<div style="display: flex; justify-content: center; align-items: center; gap: 2rem; padding: 1rem; width: 100%">
    ...
    <div>
        <h1 style="margin: 0 0 1rem 0">Weather Report Assistant</h1>
        <h3 style="margin: 0 0 0.5rem 0">
            This Weather Report Assistant answers your questions.
        </h3>
    </div>
</div>
"""
    ...
    chatbot = gr.Chatbot(
        ...
        examples=[
            {'text': 'What is your location?', 'alt_text': ''},
        ],
    )
    with gr.Row():
        prompt = gr.Textbox(
            lines=1,
            show_label=False,
            placeholder='What is the your location?',
